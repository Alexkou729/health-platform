  ...
  async sendTemplateMessage(openid, templateId, data, url) {
    ...
    try {
      ...
      await this.prisma.wxMessage.create({
        data: { openid, templateId, content: data, url, status: res.data.errcode === 0 ? 1 : 2, errorMsg: res.data.errmsg, msgId: res.data.msgid, sentAt: new Date() },
      });
      return res.data;
    } catch (e) {
      await this.prisma.wxMessage.create({ data: { openid, templateId, content: data, status: 2, errorMsg: e.message } });
      throw e;
    }
  }
