...
export class ScriptService {
  ...
  async listTemplates(params: any) {
    ...
    const pageSizeNum = Math.min(200, parseInt(String(pageSize), 10) || 50);
    const where: any = {};
    if (category) where.category = category;
    if (status) where.status = status;
    if (keyword) where.OR = [{ title: { contains: keyword } }, { code: { contains: keyword } }];
    const [items, total] = await Promise.all([
      this.prisma.scriptTemplate.findMany({ where, skip: (pageNum-1)*pageSizeNum, take: pageSizeNum, orderBy: { usageCount: "desc" } }),
      this.prisma.scriptTemplate.count({ where }),
    ]);
    return { items: items.map((i) => this.parseTemplate(i)), total, page: pageNum, pageSize: pageSizeNum };
  }
