...
export class ReportRenderer {
  ...
  async initBrowser() {
    ...
    try {
      this.puppeteerLib = this.loadPuppeteer();
      if (!this.puppeteerLib) {
        this.logger.warn('未安装 Puppeteer，PDF 将回退为 HTML 输出');
        return;
      }
      ...
      this.browser = await this.puppeteerLib.launch(launchOpts);
  ...
  /** 惰性加载 puppeteer-core 或 puppeteer，避免依赖缺失导致启动失败 */
  ...
  async renderHtml(report) {
    ...
    return '<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8">' +
      ...
      '.header { background: linear-gradient(135deg, #10b981 0%, #0ea5e9 100%); padding: 40px; border-radius: 20px; margin-bottom: 24px; position: relative; overflow: hidden; }' +
  ...
  async renderPdf(report) {
    if (!this.browser) {
      // 不把 HTML 冒充 PDF，明确返回 503
      throw new ServiceUnavailableException('PDF 渲染服务暂不可用');
    }
