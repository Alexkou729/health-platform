...
const routes = [
  { path: '/login', component: () => import('@/views/Login.vue'), meta: { layout: 'blank' } },
  {
    ...
    children: [
      ...
      { path: 'products', component: () => import('@/views/Products.vue'), meta: { title: '商品管理', icon: 'Goods' } },
      { path: 'mall-orders', component: () => import('@/views/MallOrders.vue'), meta: { title: '商城订单', icon: 'ShoppingCart' } },
      { path: 'home-service-orders', component: () => import('@/views/HomeServiceOrders.vue'), meta: { title: '上门服务', icon: 'House' } },
      ...
      { path: 'settlements', component: () => import('@/views/Settlements.vue'), meta: { title: '结算比例', icon: 'Money', roles: ['SUPER_ADMIN'] } },
